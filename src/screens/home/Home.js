...
class Home extends Component {
    ...
    async categoryArray() {
        if(this.props.TodoList.data !== 'undefined' && this.props.TodoList.data.length > 0) {
            myObj = []
            await this.props.TodoList.data.map(value => {
                myObj.push({value: value.category})
            })
            await myObj.unshift({value:'My Task'})
        } else {
            await myObj.unshift({value:'My Task'})
        }
    }
                            ...
                            style={[styles.note, {borderLeftColor:item.color,}]}>
                                ...
                                <View style={styles.noteBox} >
                                    <TouchableOpacity onPress={()=> this.props.navigation.navigate('EditForm', {id:item.id})} style={styles.noteEdit}>
                                        <Icon name='edit' type='feather' color='grey' size={20} />
                                    </TouchableOpacity>
                                    <TouchableOpacity 
                                        onPress={()=> Alert.alert(
                                            '',
                                        ...
                                        <Icon name='delete' type='antdesign' color='grey' size={20} />
                                    </TouchableOpacity>
                                </View>
                            </View>
                        )}
